...
export type BookingPdfData = {
  ...
  equipments?: string[];     // หรือ services ที่ต้องเตรียม
  agenda?: string;           // purpose / agenda
  notes?: string;
};
...
<meta charset="utf-8" />
<style>
  @font-face { font-family: 'Sarabun'; src: url('${sarabunRegular}') format('truetype'); font-weight: 400; }
  @font-face { font-family: 'Sarabun'; src: url('${sarabunBold}') format('truetype'); font-weight: 700; }
  * { box-sizing: border-box; }
  body { font-family: 'Sarabun', system-ui, -apple-system, 'Segoe UI', Roboto, sans-serif; margin: 0; padding: 24px; font-size: 12pt; color: #111; }
  ...
  .cell, .th { border: 1px solid #ccc; padding: 6px 8px; vertical-align: top; }
...
<body>
  ...
  <div class="section grid">
    ...
    <div class="box">
      <div><span class="label">วันที่-เวลาเริ่ม:</span><span class="value">${start.format("D MMM BBBB เวลา HH:mm")}</span></div>
      <div><span class="label">วันที่-เวลาสิ้นสุด:</span><span class="value">${end.format("D MMM BBBB เวลา HH:mm")}</span></div>
      <div><span class="label">ระยะเวลา:</span><span class="value">${end.diff(start, "minute")} นาที</span></div>
    </div>
  </div>
  ...
  <div class="section">
    <div class="label">อุปกรณ์/บริการที่ต้องเตรียม</div>
